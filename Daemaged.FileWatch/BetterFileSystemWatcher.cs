...
{
  public class BetterFileSystemWatcher : FileSystemWatcher, IDisposable
  {
    ...
    public BetterFileSystemWatcher() { InitializeMembers(); }

    public BetterFileSystemWatcher(string Path)
      : base(Path) { InitializeMembers(); }

    public BetterFileSystemWatcher(string Path, string Filter)
      : base(Path, Filter) { InitializeMembers(); }
    ...
    protected new virtual void OnChanged(FileSystemEventArgs e) { if (Changed != null) Changed(this, e); }

    protected new virtual void OnCreated(FileSystemEventArgs e) { if (Created != null) Created(this, e); }

    protected new virtual void OnDeleted(FileSystemEventArgs e) { if (Deleted != null) Deleted(this, e); }

    protected new virtual void OnRenamed(RenamedEventArgs e) { if (Renamed != null) Renamed(this, e); }
    ...
    private bool HasAnotherFileEventOccuredRecently(string FileName)
    {
      ...
      if (FilterRecentEvents) {
        if (_lastFileEvent.ContainsKey(FileName)) {
          ...
          // and and filename along with current datetime to the dictionary
